...
*Austin Mabry station was chosen because it has a good number of data points (78) and because it has data that is consistently sampled i.e. no breaks in years between datapoints. The location (austin) is also known for flash floods and i suspect that we may obverve information from the plots about its rainfall risks.*

**Response 2:** Fitting Methods Comparison  
*
The parameters of all three approaches are very similar. Each one of the values of \sigma, \delta and \epsilon are extremely close to each other. The GEV plots comparing the fits also highlight that the values follow the same trend.*

**Response 3:** Multi-Station Geographic Analysis  
*
The parameters for four of the five nearby stations sampled are quite similar. However the fifth station (BARTON CRK AT LOOP 360) displays a higher number of high rainfall events likely because of a higher altitude. It could also be due to a higher influence of the Balcones Escarpment, a fault line that is responible for weather phenomenon in the area. In terms of reliability, as there are only ~30 data points for that particular station, the estimation may not be representative of the underlying reality.*

**Response 4:** Uncertainty and Sample Size Insights  
*
From the comparative plots of 30 curves from different sub-samples, it looks like some of the fit curves are deviating a lot from the true distribution. This seems to be an issue with the sampling cardinality as more events (100) seems to have lower deviation from the mean compared to less samples (30). In my opinion, we need to find stations with more than 100 years of data atleast and if not, at minimum more than 50 years of data to estimate the true GEV curve.*

**Response 5:** Final Synthesis  
*In my opinion, the data that most weather stations have (less than or ~100) is insufficient to draw claims about the GEV characgteristics of the area's weather. I believe that both topological and geographical features should be used along with aggregated data from a group of weather stations of a region (neighbours) to get a true bearing of the rain characteristics of a region. The GEV of neighbours may also be found as a shared parameter co-optimization problem that together minimizes the weighted MLE/MAP loss of the neighbours GEV prediction losses.*
